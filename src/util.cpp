...
void *processSingleMisWork(void *arg){
	misWork_opt *miswork_opt = (misWork_opt *)arg;
	miswork *mis_work = miswork_opt->mis_work;
	size_t num_done, num_work, num_work_percent;
	double percentage;
	Time time;

	mis_work->getindicator();

	pthread_mutex_lock(miswork_opt->p_mtx_misworkDone_num);
	(*miswork_opt->p_misworkDone_num) ++;
	num_done = *miswork_opt->p_misworkDone_num;

	num_work = miswork_opt->num_work;
	num_work_percent = miswork_opt->num_work_percent;
	...
	pthread_mutex_unlock(miswork_opt->p_mtx_misworkDone_num);

	delete (misWork_opt *)arg;
